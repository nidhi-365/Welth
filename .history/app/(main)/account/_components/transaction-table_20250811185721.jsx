...
const [recurringFilter,setRecurringFilter]=useState("");
  ...
  const handleSelectAll = () =>{setSelectedIds((current)=>current.length===filteredAndSortedTransactions.length ? []:filteredAndSortedTransactions.map(t=>t.id))};
    ...
    <div className='space-y-4'>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
                <Search className="absolute left2 top-2.5 h-4 w-4 text-muted-foreground" />
            ...
            <div className="flex gap-2">
                ...
                {(searchTerm || typeFilter || recurringFilter) && <Button variant="outline" size="icon" onClick={handleClearFilters} title="Clear Filters"><X className="h-4 w-5"/></Button>}
            </div>
        </div>
        {/*Transactions */}
  ...
  <TooltipTrigger>
    <Badge variant="outline" className="gap-1">
        <RefreshCw className="gap-1 bg-purple-100 text-purple-700 hover:bg-purple-200"/>{RECURRING_INTERVALS[transaction.recurringInterval]}</Badge>
  </TooltipTrigger>
  <TooltipContent>
